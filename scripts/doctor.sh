...
st="$(docker inspect -f '{{.State.Health.Status}}' gardik-db 2>/dev/null || true)"
...
if [ "$do_warn" = "1" ]; then
  ...
  if [ "$fx_dbup" = "1" ]; then
    ...
    i=0
    while [ "$(docker inspect -f '{{.State.Health.Status}}' gardik-db 2>/dev/null || true)" != "healthy" ] && [ "$i" -lt 60 ]; do
      i=$((i + 1)); sleep 1
    done
    applied=1
  fi
