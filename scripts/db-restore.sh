...
if [ "$(docker inspect -f '{{.State.Health.Status}}' gardik-db 2>/dev/null)" != "healthy" ]; then
...
docker compose exec -T db pg_restore --clean --if-exists -U "${POSTGRES_USER:-user}" -d "${POSTGRES_DB:-gardik}" < "$file"
